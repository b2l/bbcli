...
import { loadConfig, ConfigError } from "./index.ts";
...
describe("loadConfig", () => {
  ...
  test("rejects missing config file", async () => {
    ...
    expect(msg).toMatchInlineSnapshot(`"No config file found at does-not-exist.json."`);
  });
  ...
  test("rejects both token and token_command", async () => {
    ...
    expect(msg).toMatchInlineSnapshot(`"Invalid config in both-set.json: ✖ Exactly one of "token" or "token_command" must be set."`);
  });

  test("rejects neither token nor token_command", async () => {
    const msg = await expectConfigError("neither-set.json");
    expect(msg).toMatchInlineSnapshot(`"Invalid config in neither-set.json: ✖ Exactly one of "token" or "token_command" must be set."`);
  });
  ...
  test("rejects token_command that exits non-zero", async () => {
    ...
    expect(msg).toMatchInlineSnapshot(`"token_command exited with code 1: (no output)"`);
  });
  ...
  test("rejects token_command with nonexistent binary", async () => {
    const err = await loadConfig(fixture("command-not-found.json")).catch((e) => e);
    expect(err).toBeInstanceOf(ConfigError);
  });
});
